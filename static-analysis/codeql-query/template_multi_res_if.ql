...
from DataFlow::Node source, DataFlow::Node sink, int i, Function f, IfStmt ifs
where IndirectionCallFlow::flow(source, sink)
    ...
    and
    ifs = sink.asExpr().getParent*()
//select sink, i.toString() + " | " + ifs.getLocation().getFile().toString() + " | " + ifs.getLocation().getStartLine()
