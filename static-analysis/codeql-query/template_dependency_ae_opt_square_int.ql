...
predicate ifReachableFromRelevantStatementFast(IfStmt ifs, ValidOpts a) {
 ...
 /*
 i'm thinking: have the kind of sockopt and the entry functions as predicates in the class
 subclasses for if and fc, and for both set and get
 the leaf classes specify entry points, kind of sockopt, whatever else is required so that all predicates can use it and we can print the required info in the result
 */
 ...
 from AssignExpr ae,
   ...
   where 
       (
        ...
        and
        aeReachableFromRelevantStatementFast(ae, vof)   
       )
 select ifs.getLocation(), l2.getValue() + " | " + (s.getName() + ":" + pfa.getTarget().getName()) + " | " + vof.getvalue().toString() + " | " + voi.getvalue().toString()  + " | " + ifs.getLocation().getFile().toString() + " | " + ifs.getLocation().getStartLine().toString() + " | " + ae.getLocation().getFile().toString() + " | " + ae.getLocation().getStartLine().toString() + " | " + vof.getkind() + " | " + voi.getkind()
