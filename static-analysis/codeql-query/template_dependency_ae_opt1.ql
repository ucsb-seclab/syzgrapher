predicate relevantStatementRec(Stmt s, ValidOpts a) {
  a.checkCase(s.(SwitchCase)) //s.(SwitchCase).getExpr().(Literal).getValue() = a.toString()
    or
    (
...
select ifs.getLocation(), f2.getName() + " | "  + eca2.getValue() + " | " + eca2.toString() + " | " + va.getvalue().toString() + " | " + ifs.getLocation().getFile().toString() + " | " + ifs.getLocation().getStartLine().toString() + " | " + ae.getLocation().getFile().toString() + " | " + ae.getLocation().getStartLine().toString()