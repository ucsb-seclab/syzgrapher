...
// Contains extra information about a given input, used for history tracking.
// This struct is serialized and stored in meta.db
type InputMeta struct {
	Prog        []byte
	ID          uuid.UUID
	Origin      *prog.ProgOrigin
	CreatedAt   int64 // Unix timestamp in milliseconds (set during save)
	SignalCalls []int
	SignalExtra bool
}

type LogMessageArgs struct {
	Name string
	Text string
}
...
type StaticGCTArgs struct {
	Name string
}

type StaticGCTRes struct {
