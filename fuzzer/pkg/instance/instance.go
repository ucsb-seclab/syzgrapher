...
type OptionalFuzzerArgs struct {
	...
	ProgSampleFreq    float64
	DisableGCT        bool
	DisableDynamicGCT bool
	MaxProgramSize    int
	FreezeAfterTriage bool
}
...
func FuzzerCmd(args *FuzzerCmdArgs) string {
	...
	if args.Optional != nil {
		flags := []tool.Flag{
			...
			{Name: "log_mutations", Value: fmt.Sprint(args.Optional.LogMutations)},
			{Name: "syzgrapher_freq", Value: fmt.Sprint(args.Optional.SyzgrapherFreq)},
			{Name: "trace_history", Value: fmt.Sprint(args.Optional.TraceHistory)},
			{Name: "advanced_metrics", Value: fmt.Sprint(args.Optional.AdvancedMetrics)},
			{Name: "prog_sample_freq", Value: fmt.Sprint(args.Optional.ProgSampleFreq)},
			{Name: "disable_gct", Value: fmt.Sprint(args.Optional.DisableGCT)},
			{Name: "disable_dynamic_gct", Value: fmt.Sprint(args.Optional.DisableDynamicGCT)},
			{Name: "max_program_size", Value: fmt.Sprint(args.Optional.MaxProgramSize)},
			{Name: "freeze_after_triage", Value: fmt.Sprint(args.Optional.FreezeAfterTriage)},
		}
