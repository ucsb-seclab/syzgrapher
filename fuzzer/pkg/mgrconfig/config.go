...
type Config struct {
	...
	// Config file for schema
	SchemaFile string `json:"schema_file"`

	// If true, logs verbose mutation information.
	LogMutations bool `json:"log_mutations"`

	// Frequency of programs to mutate with syzgrapher
	SyzgrapherFreq float64 `json:"syzgrapher_freq"`

	// If true, keep track of program origin history.
	TraceHistory bool `json:"trace_history"`

	// If true, reports detailed metrics about the fuzzing process.
	AdvancedMetrics bool `json:"advanced_metrics"`

	// How frequently to sample random programs for inclusion in meta db (requires trace_history).
	ProgSampleFreq float64 `json:"prog_sample_freq"`

	// If true, disable the graph choice table during mutations.
	DisableGCT bool `json:"disable_gct"`

	DisableDynamicGCT bool `json:"disable_dynamic_gct"`

	MaxProgramSize int `json:"max_program_size"`

	FreezeAfterTriage bool `json:"freeze_after_triage"`
