...
type MutatorOptions struct {
	Verbose        bool
	SyzgrapherFreq float64
	TraceHistory   bool
	DisableGCT     bool
	MaxProgramSize int
}

func DefaultMutatorOptions() MutatorOptions {
...
// schema:      The schema of the target.
// verbose:     Whether to print debug information.
// syzgrapherFreq: The frequency of using syzgrapher mutations.
// traceHistory: Whether to trace the history of the mutations.
func (p *Prog) Mutate(rs rand.Source, ncalls int, ct *ChoiceTable, noMutate map[int]bool,
	corpus []*Prog, schema *TargetSchema, opts MutatorOptions, gct *GraphChoiceTable) *Prog {
	r := newRand(p.Target, rs)
	if schema != nil && r.Float64() < opts.SyzgrapherFreq {
		var history *OriginMutatedGraph = nil
		if opts.TraceHistory {
		...
		if ctx.Verbose {
			data := p.Serialize()
			fmt.Printf("[Graph before mutation]:\n%s\n", data)
		}

		// clone := p.Clone()
		graph := ProgToGraph(p)
		// if graph.HasSelfReference() {
		// 	// This can occur in rare cases when syzkaller mutations produce something like:
		// 	// r0 = open(...)
		// 	// r1 = dup2(r0, r0)
		// 	//
		// 	// These cases are not natively representable in the graph format (without dummy copy nodes).
		// 	if ctx.Verbose {
		// 		fmt.Printf("Graph has self reference, skipping mutation\n")
		// 	}
		// 	return clone
		// }
		graph.Mutate(ctx)
		mutatedProg := GraphToProg(graph)

		if opts.TraceHistory {
			mutatedProg.NewID()
			mutatedProg.SetOrigin(*history)
		}

		mutatedProg.sanitizeFix()
		mutatedProg.debugValidate()
		return mutatedProg
	}
	if ncalls < len(p.Calls) {
		ncalls = len(p.Calls)
	}
	var history *OriginMutated = nil
	if opts.TraceHistory {
		history = &OriginMutated{
			Parent: p.ID,
		}
	}
	ctx := &mutator{
...
func (p *Prog) WrappedMutateArg(rs rand.Source, ncalls int, ct *ChoiceTable, noMutate map[int]bool, corpus []*Prog) {
	r := newRand(p.Target, rs)
	ctx := &mutator{
		p:        p,
		r:        r,
...
func (ctx *mutator) splice() bool {
	...
	if ctx.history != nil {
		ctx.history.Steps = append(ctx.history.Steps, MutationStepSplice{
			Other:     p0.ID,
			OtherSize: len(p0.Calls),
			Index:     idx,
			Truncated: tcount,
		})
	}
...
func (ctx *mutator) squashAny() bool {
	...
	arg.Dat = mutateData(r, arg.Data(), 0, maxBlobLen)
	...
	if ctx.history != nil {
		ctx.history.Steps = append(ctx.history.Steps, MutationStepSquashAny{})
	}
	return true
}
...
func (ctx *mutator) insertCall() bool {
	...
	if ctx.history != nil {
		ctx.history.Steps = append(ctx.history.Steps, MutationStepInsertCall{
			CallIndex:       idx,
			SyscallID:       calls[len(calls)-1].Meta.ID,
			ConstructorSize: len(calls) - 1,
			Truncated:       tcount,
		})
	}
	return true
...
func (ctx *mutator) removeCall() bool {
	...
	p.RemoveCall(idx)
	if ctx.history != nil {
		ctx.history.Steps = append(ctx.history.Steps, MutationStepRemoveCall{
			CallIndex: idx,
		})
	}
	return true
...
func (ctx *mutator) mutateArg() bool {
	...
	if ctx.history != nil {
		ctx.history.Steps = append(ctx.history.Steps, MutationStepMutateArg{
			CallIndex: idx,
		})
	}
	return true
...
func (t *BufferType) mutate(r *randGen, s *state, arg Arg, ctx ArgCtx) (calls []*Call, retry, preserve bool) {
	...
	if a.GetDir() == DirOut {
		if t.Kind == BufferFilename && r.oneOf(100) {
			a.Siz = uint64(r.randFilenameLength())
		} else {
	...
	case BufferBlobRand, BufferBlobRange:
		...
		a.Dat = mutateData(r, data, minLen, maxLen)
	case BufferString:
		if len(t.Values) != 0 {
			a.Dat = r.randString(s, t)
		} else {
			if t.TypeSize != 0 {
	...
	case BufferText:
		data := append([]byte{}, a.Data()...)
		a.Dat = r.mutateText(t.Text, data)
	case BufferCompressed:
		a.Dat, retry = r.mutateImage(a.Data())
	default:
...
func mutateBufferSize(r *randGen, arg *DataArg, minLen, maxLen uint64) {
	for oldSize := arg.Size(); oldSize == arg.Size(); {
		arg.Siz += uint64(r.Intn(33)) - 16
		// Cast to int64 to prevent underflows.
		if int64(arg.Siz) < int64(minLen) {
			arg.Siz = minLen
		}
		if arg.Siz > maxLen {
			arg.Siz = maxLen
