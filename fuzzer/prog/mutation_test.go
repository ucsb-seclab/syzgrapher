	...
	next:
		for i := 0; i < iters; i++ {
			...
			for try := 0; try < 20; try++ {
				p1.Mutate(rs, 10, ct, nil, nil, nil, DefaultMutatorOptions())
				data := p.Serialize()
...
func TestMutateCorpus(t *testing.T) {
	...
	for i := 0; i < iters; i++ {
		...
		p1.Mutate(rs, 10, ct, nil, corpus, nil, DefaultMutatorOptions())
	}
}
...
func BenchmarkMutate(b *testing.B) {
	...
	b.RunParallel(func(pb *testing.PB) {
		rs := rand.NewSource(0)
		for pb.Next() {
			p.Clone().Mutate(rs, progLen, ct, nil, nil, nil, DefaultMutatorOptions())
		}
	})
}
...
func runMutationTests(t *testing.T, tests [][2]string, valid bool) {
	...
	for ti, test := range tests {
		...
		t.Run(fmt.Sprint(ti), func(t *testing.T) {
			...
			for i := 0; i < iters; i++ {
				p1 := p.Clone()
				p1.Mutate(rs, len(goal.Calls), ct, nil, nil, nil, DefaultMutatorOptions())
				data1 := p1.Serialize()
				if bytes.Equal(want, data1) {
					if !valid {
