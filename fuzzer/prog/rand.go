...
func newRandExisting(target *Target, rand *rand.Rand) *randGen {
	return &randGen{
		Rand:     rand,
		target:   target,
		recDepth: make(map[string]int),
...
func GenerateParticularCall(ct *ChoiceTable, meta *Syscall, t *Target) (calls []*Call) {
	s := newState(t, ct, nil)
	r := newRand(t, rand.NewSource(time.Now().UnixNano()))
	return r.generateParticularCall(s, meta)
}
