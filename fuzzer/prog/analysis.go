...
func (s *stateWrapper) analyzeArgs(arg Arg, _ *ArgCtx) {
	s.s.analyzeArgsImpl(arg, nil, s.resources)
}

func (s *state) analyzeArgsImpl(arg Arg, _ *ArgCtx, resources bool) {
	switch a := arg.(type) {
	case *PointerArg:
		switch {
		case a.IsSpecial():
