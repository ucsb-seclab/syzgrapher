...
func (ctx *validCtx) validateArg(arg Arg, typ Type, dir Dir) error {
	...
	if arg.GetDir() != dir && dir != DirInOut {
...
func (arg *ResultArg) validate(ctx *validCtx, dir Dir) error {
	...
	if arg.GetDir() == DirIn && len(arg.uses) > 0 {
...
func (arg *DataArg) validate(ctx *validCtx, dir Dir) error {
	...
	if arg.GetDir() == DirOut && len(arg.Dat) != 0 {
	...
	case BufferFilename:
		if escapingFilename(string(arg.Dat)) {
			return fmt.Errorf("escaping filename %q", arg.Dat)
		}
	}
	return nil
}
