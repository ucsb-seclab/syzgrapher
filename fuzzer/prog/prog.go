...
type DataArg struct {
	ArgCommon
	Dat []byte // for in/inout args
	Siz uint64 // for out Args
}
...
func MakeDataArg(t Type, dir Dir, data []byte) *DataArg {
	...
	return &DataArg{ArgCommon: ArgCommon{Ref: t.ref(), Dir: dir}, Dat: append([]byte{}, data...)}
}
...
func MakeOutDataArg(t Type, dir Dir, size uint64) *DataArg {
	...
	return &DataArg{ArgCommon: ArgCommon{Ref: t.ref(), Dir: dir}, Siz: size}
}

func (arg *DataArg) Size() uint64 {
	if len(arg.Dat) != 0 {
		return uint64(len(arg.Dat))
	}
	return arg.Siz
}
...
type ResultArg struct {
	...
	out   bool                // used by syzgrapher to see whether an inout resource is also an out
}

func AddUses(r1, r2 *ResultArg) {
	if r1.uses == nil {
		r1.uses = make(map[*ResultArg]bool)
	}
	r1.uses[r2] = true
}
...
func (p *Prog) SetOrigin(origin ProgOrigin) {
	p.Origin = &origin
}
