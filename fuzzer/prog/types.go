...
// PseudoType that carries information about a destructor to the field generation code.
// This type resolves to the inner type with Dir=del.
type DelPseudoType struct {
	TypeCommon
	Elem Type
}

func (t *DelPseudoType) String() string {
	return fmt.Sprintf("del[%v]", t.Elem.String())
}

func (t *DelPseudoType) DefaultArg(dir Dir) Arg {
	return t.Elem.DefaultArg(DirIn)
}

func (t *DelPseudoType) isDefaultArg(arg Arg) bool {
	return t.Elem.isDefaultArg(arg)
}
