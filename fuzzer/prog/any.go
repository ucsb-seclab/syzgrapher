...
func (target *Target) squashPtrImpl(a Arg, elems *[]Arg) {
	...
	case *DataArg:
		if arg.GetDir() == DirOut {
			...
			elem.Dat = append(elem.Data(), arg.Data()...)
		}
	case *GroupArg:
	...
	if pad != 0 {
		elem := target.ensureDataElem(elems)
		elem.Dat = append(elem.Data(), make([]byte, pad)...)
	}
}
...
func (target *Target) squashConst(arg *ConstArg, elems *[]Arg) {
	if IsPad(arg.Type()) {
		elem := target.ensureDataElem(elems)
		elem.Dat = append(elem.Data(), make([]byte, arg.Size())...)
		return
	}
	...
	elem := target.ensureDataElem(elems)
	elem.Dat = append(elem.Data(), data...)
}
		...
		if fld.Type().IsBitfield() {
			...
			if fld.Size() != 0 {
				...
				for i := uint64(0); i < fld.Size(); i++ {
					elem.Dat = append(elem.Data(), byte(bitfield))
					bitfield >>= 8
				}
				bitfield = 0
			}
