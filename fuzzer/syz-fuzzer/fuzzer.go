...
var (
	flagDisableDynamicGCT = flag.Bool("disable_dynamic_gct", false, "disable dynamic graph choice table")
)
...
type Fuzzer struct {
	...
	// syzgrapher fields
	schema          *prog.TargetSchema
	mutatorStats    *prog.MutatorStats
	collisionFilter *bloom.BloomFilter
	staticGCT       *prog.StaticGCT
	gct             *prog.GraphChoiceTable
}
...
func main() {
	...
	fuzzer := &Fuzzer{
		...
		mutatorStats:             prog.NewMutatorStats(),
	}
	if *flagAdvancedMetrics && *flagTraceHistory {
		// Collision filter to estimate mutation collision rates.
		fuzzer.collisionFilter = bloom.NewWithEstimates(10000000, 0.0001)
	}
	...
	// Fetch schema from manager and apply syscall filter.
	fmt.Printf("Fetching schema from manager...\n")
	fuzzer.getSchema()
	enabledCalls := make(map[int]bool)
	for i := range fuzzer.target.Syscalls {
		enabledCalls[i] = fuzzer.choiceTable.Enabled(i)
	}
	fmt.Printf("Applying syscall filter...\n")
	fuzzer.schema.ApplySyscallFilter(enabledCalls)
	fuzzer.schema.PrintEnabled()

	fmt.Printf("Fetching static graph choice table from manager...\n")
	fuzzer.getStaticGCT()
	if fuzzer.staticGCT != nil {
		fmt.Printf("Parsing static graph choice table...\n")
		fuzzer.gct = fuzzer.schema.BuildGraphChoiceTable(fuzzer.corpus, fuzzer.staticGCT, *flagDisableDynamicGCT)
	}
	fmt.Printf("done\n")
...
func (fuzzer *Fuzzer) getSchema() {
	a := &rpctype.SchemaArgs{
		Name: fuzzer.name,
	}
	r := &rpctype.SchemaRes{}
	if err := fuzzer.manager.Call("Manager.GetSchema", &a, r); err != nil {
		log.SyzFatalf("Manager.GetSchema call failed: %v", err)
	}
	fuzzer.schema = r.Schema
	fuzzer.schema.SetTarget(fuzzer.target)
}

func (fuzzer *Fuzzer) getStaticGCT() {
	a := &rpctype.StaticGCTArgs{
		Name: fuzzer.name,
	}
	r := &rpctype.StaticGCTRes{}
	if err := fuzzer.manager.Call("Manager.GetStaticGCT", &a, r); err != nil {
		log.SyzFatalf("Manager.GetStaticGCT call failed: %v", err)
	}
	fuzzer.staticGCT = r.StaticGCT
}
...
func (fuzzer *Fuzzer) poll(needCandidates bool, stats map[string]uint64) bool {
	a := &rpctype.PollArgs{
		...
		Stats:          stats,
		MutatorStats:   fuzzer.mutatorStats.CloneReset(),
	}
	r := &rpctype.PollRes{}
...
func (fuzzer *Fuzzer) sendInputMetaToManager(meta rpctype.InputMeta) {
	a := &rpctype.NewInputMetaArgs{
		Name:      fuzzer.name,
		InputMeta: meta,
	}
	if err := fuzzer.manager.Call("Manager.NewInputMeta", a, nil); err != nil {
		log.SyzFatalf("Manager.NewInputMeta call failed: %v", err)
	}
}

func (fuzzer *Fuzzer) logMessage(format string, args ...interface{}) {
	msg := fmt.Sprintf(format, args...)
	a := &rpctype.LogMessageArgs{
		Name: fuzzer.name,
		Text: msg,
	}
...
func (fuzzer *Fuzzer) checkDisabledCalls(p *prog.Prog) {
	for _, call := range p.Calls {
		if !fuzzer.choiceTable.Enabled(call.Meta.ID) {
			...
			data := p.Serialize()
			fmt.Printf("Prog:\n%v\n", string(data))
			fmt.Printf("Offending call: %v (%v)\n", call.Meta, call.Meta.Name)
