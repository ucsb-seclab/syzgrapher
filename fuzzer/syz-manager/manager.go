...
type Manager struct {
	...
	serv           *RPCServer
	corpusDB       *db.DB
	metaDB         *db.DB // Used only if if TraceHistory is set
	metaDBindex    uint32 // tracks which version of the database we are on
	startTime      time.Time
	...
	mutatorInfoProm *prog.MutatorInfoProm
}
...
func RunManager(cfg *mgrconfig.Config) {
	...
	mgr := &Manager{
		...
		mutatorInfoProm:  prog.NewMutatorInfoProm(),
	}
	...
	if mgr.cfg.AdvancedMetrics {
		mgr.mutatorInfoProm.Register()
		mgr.mutatorInfoProm.Reset()
	}
	...
	log.Logf(0, "[+] syzgrapher_freq: %v", mgr.cfg.SyzgrapherFreq)
	log.Logf(0, "[+] trace_history: %v", mgr.cfg.TraceHistory)
	log.Logf(0, "[+] advanced_metrics: %v", mgr.cfg.AdvancedMetrics)
	log.Logf(0, "[+] log_mutations: %v", mgr.cfg.LogMutations)
	log.Logf(0, "[+] disable_gct: %v", mgr.cfg.DisableGCT)
	log.Logf(0, "[+] disable_dynamic_gct: %v", mgr.cfg.DisableDynamicGCT)
	log.Logf(0, "[+] max_program_size: %v", mgr.cfg.MaxProgramSize)

	if mgr.cfg.FreezeAfterTriage {
		log.Logf(0, "[+] freeze_after_triage: enabled")
	}

	if !mgr.cfg.DisableGCT {
		mgr.serv.staticGCT = syzgrapher.ParseDepInfo(mgr.cfg.Workdir, mgr.target)
	}

	if mgr.cfg.TraceHistory {
		mgr.newMetaDB(0)
	}
...
func (mgr *Manager) newMetaDB(idx uint32) {
	mgr.metaDBindex = idx

	var err error
	mgr.metaDB, err = db.Open(filepath.Join(mgr.cfg.Workdir, fmt.Sprintf("meta_%d.db", idx)), true)
	if err != nil {
		log.Fatalf("failed to open meta database: %v", err)
	}
}
...
func (mgr *Manager) runInstanceInner(index int, instanceName string) (*report.Report, []byte, error) {
	...
	args := &instance.FuzzerCmdArgs{
		...
		Optional: &instance.OptionalFuzzerArgs{
			...
			LogMutations:      mgr.cfg.LogMutations,
			SyzgrapherFreq:    mgr.cfg.SyzgrapherFreq,
			TraceHistory:      mgr.cfg.TraceHistory,
			AdvancedMetrics:   mgr.cfg.AdvancedMetrics,
			DisableGCT:        mgr.cfg.DisableGCT,
			MaxProgramSize:    mgr.cfg.MaxProgramSize,
			FreezeAfterTriage: mgr.cfg.FreezeAfterTriage,
		},
	}
...
func (mgr *Manager) newInputMeta(meta rpctype.InputMeta) bool {
	mgr.mu.Lock()
	defer mgr.mu.Unlock()

	// if meta.Origin == nil {
	// 	log.Logf(0, "new input meta: <nil>")
	// } else {
	// 	json, err := json.Marshal(meta.Origin)
	// 	if err != nil {
	// 		log.Logf(0, "new input meta: %v", err)
	// 	} else {
	// 		log.Logf(0, "new input meta: %s", json)
	// 	}
	// }
	...
	sig := hash.String(buf.Bytes())
	mgr.metaDB.Save(sig, buf.Bytes(), 0)
	if err := mgr.metaDB.Flush(); err != nil {
		log.Errorf("failed to save meta database: %v", err)
	}

	// Roll over to a new database after 100,000 entries
	if len(mgr.metaDB.Records) >= 100000 {
		mgr.newMetaDB(mgr.metaDBindex + 1)
	}
