...
type RPCManagerView interface {
	...
	newInputMeta(inp rpctype.InputMeta) bool
...
func startRPCServer(mgr *Manager) (*RPCServer, error) {
	serv := &RPCServer{
		...
		mutatorInfoProm: mgr.mutatorInfoProm,
	}

	// load schemafile if present
	content, err := ioutil.ReadFile(serv.cfg.SchemaFile)
	if err != nil {
		log.Logf(0, "No schema found")
		serv.schema = nil
	} else {
...
func (serv *RPCServer) rotateCorpus(f *Fuzzer, corpus []rpctype.Input) *rpctype.CheckArgs {
	...
	// Note: GCT filtering happens in the fuzzer process
	// f.gct = serv.mgr.rotateGCT(calls)
	//f.gct = prog.FilterGCT(serv.gct, calls, serv.mgr.target)
...
func (serv *RPCServer) Check(a *rpctype.CheckArgs, r *int) error {
	...
	if serv.schema == nil {
		serv.schema = serv.cfg.Target.ComputeBaseSchema()
	}
	log.Logf(0, "schema signatures: %v", len(serv.schema.Signatures))
	serv.persistSchema()
...
func (serv *RPCServer) NewInputMeta(a *rpctype.NewInputMetaArgs, r *int) error {
	serv.mu.Lock()
	defer serv.mu.Unlock()

	serv.mgr.newInputMeta(a.InputMeta)
	return nil
}

func (serv *RPCServer) LogMessage(a *rpctype.LogMessageArgs, r *int) error {
	log.Logf(0, "fuzzer %v: %v", a.Name, a.Text)
	return nil
}

func (serv *RPCServer) GetSchema(a *rpctype.SchemaArgs, r *rpctype.SchemaRes) error {
	serv.mu.Lock()
	defer serv.mu.Unlock()
	...
	r.Schema = serv.schema

	return nil
}

func (serv *RPCServer) GetStaticGCT(a *rpctype.StaticGCTArgs, r *rpctype.StaticGCTRes) error {
	serv.mu.Lock()
	defer serv.mu.Unlock()

	r.StaticGCT = serv.staticGCT
...
func (serv *RPCServer) Poll(a *rpctype.PollArgs, r *rpctype.PollRes) error {
	...
	serv.mutatorInfoProm.Observe(&a.MutatorStats)
...
func (serv *RPCServer) persistSchema() {
	var buf bytes.Buffer
	enc := gob.NewEncoder(&buf)
	if err := enc.Encode(serv.schema); err != nil {
		log.Fatalf("failed to encode schema: %v", err)
	}
	if err := ioutil.WriteFile(serv.cfg.SchemaFile, buf.Bytes(), 0660); err != nil {
